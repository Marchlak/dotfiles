...
i3-msg 'workspace number 9; exec --no-startup-id brave-browser --app=https://teams.microsoft.com' >/dev/null
...
i3-msg '[class="(?i)brave.*" title="(?i).*teams.*"] floating disable' >/dev/null
i3-msg '[class="(?i)brave.*" title="(?i).*teams.*"] move container to workspace number 9' >/dev/null
i3-msg '[class="(?i)brave.*" title="(?i)^(?!.*teams).*$"] move container to workspace number 2' >/dev/null

sleep 1
log "final-info: $(ghostty_info | tr -d '\n')"
log "end"
